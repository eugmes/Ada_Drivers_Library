...
package STM32_SVD.MPU is
   ...
   type MPU_Peripheral is record
      ...
      pragma Volatile_Full_Access (MPU_TYPER);
      --  MPU control register
      MPU_CTRL  : aliased MPU_CTRL_Register;
      pragma Volatile_Full_Access (MPU_CTRL);
      --  MPU region number register
      MPU_RNR   : aliased MPU_RNR_Register;
      pragma Volatile_Full_Access (MPU_RNR);
      --  MPU region base address register
      MPU_RBAR  : aliased MPU_RBAR_Register;
      pragma Volatile_Full_Access (MPU_RBAR);
      --  MPU region attribute and size register
      MPU_RASR  : aliased MPU_RASR_Register;
      pragma Volatile_Full_Access (MPU_RASR);
   end record
     with Volatile;
